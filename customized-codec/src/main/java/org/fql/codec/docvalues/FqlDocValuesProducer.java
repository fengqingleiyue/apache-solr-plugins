 ...
 * see {@link org.apache.lucene.codecs.lucene70.Lucene70DocValuesProducer}
 */
final class FqlDocValuesProducer extends DocValuesProducer implements Closeable {
    ...
    FqlDocValuesProducer(SegmentReadState state, String dataCodec, String dataExtension, String metaCodec, String metaExtension) throws IOException {
        ...
        try (ChecksumIndexInput in = state.directory.openChecksumInput(metaName, state.context)) {
            ...
            try {
                version = CodecUtil.checkIndexHeader(in, metaCodec,
                        FqlDocValuesFormat.VERSION_START,
                        FqlDocValuesFormat.VERSION_CURRENT,
    ...
    private void readFields(ChecksumIndexInput meta, FieldInfos infos) throws IOException {
        for (int fieldNumber = meta.readInt(); fieldNumber != -1; fieldNumber = meta.readInt()) {
            ...
            if (type == FqlDocValuesFormat.NUMERIC) {
                numerics.put(info.name, readNumeric(meta));
            } else if (type == FqlDocValuesFormat.BINARY) {
                binaries.put(info.name, readBinary(meta));
            } else if (type == FqlDocValuesFormat.SORTED) {
                sorted.put(info.name, readSorted(meta));
            } else if (type == FqlDocValuesFormat.SORTED_SET) {
                sortedSets.put(info.name, readSortedSet(meta));
            } else if (type == FqlDocValuesFormat.SORTED_NUMERIC) {
