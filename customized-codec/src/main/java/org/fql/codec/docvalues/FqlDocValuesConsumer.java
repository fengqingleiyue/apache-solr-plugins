...
import static org.fql.codec.docvalues.FqlDocValuesFormat.NUMERIC_BLOCK_SIZE;
import static org.fql.codec.docvalues.FqlDocValuesFormat.NUMERIC_BLOCK_SHIFT;
import static org.fql.codec.docvalues.FqlDocValuesFormat.DIRECT_MONOTONIC_BLOCK_SHIFT;
/**
 * Created by fengqinglei on 2019/11/27.
 * see {@link org.apache.lucene.codecs.lucene70.Lucene70DocValuesConsumer}
 */
final class FqlDocValuesConsumer extends DocValuesConsumer implements Closeable {

    IndexOutput data, meta;
    final int maxDoc;

    /** expert: Creates a new writer */
    public FqlDocValuesConsumer(SegmentWriteState state, String dataCodec, String dataExtension, String metaCodec, String metaExtension) throws IOException {
        boolean success = false;
        try {
            String dataName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix, dataExtension);
            data = state.directory.createOutput(dataName, state.context);
            CodecUtil.writeIndexHeader(data, dataCodec, FqlDocValuesFormat.VERSION_CURRENT, state.segmentInfo.getId(), state.segmentSuffix);
            String metaName = IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix, metaExtension);
            meta = state.directory.createOutput(metaName, state.context);
            CodecUtil.writeIndexHeader(meta, metaCodec, FqlDocValuesFormat.VERSION_CURRENT, state.segmentInfo.getId(), state.segmentSuffix);
    ...
    public void addNumericField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        ...
        meta.writeByte(FqlDocValuesFormat.NUMERIC);
    ...
    public void addBinaryField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(FqlDocValuesFormat.BINARY);
    ...
    public void addSortedField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(FqlDocValuesFormat.SORTED);
    ...
    private void addTermsDict(SortedSetDocValues values) throws IOException {
        ...
        meta.writeInt(FqlDocValuesFormat.TERMS_DICT_BLOCK_SHIFT);

        RAMOutputStream addressBuffer = new RAMOutputStream();
        meta.writeInt(DIRECT_MONOTONIC_BLOCK_SHIFT);
        long numBlocks = (size + FqlDocValuesFormat.TERMS_DICT_BLOCK_MASK) >>> FqlDocValuesFormat.TERMS_DICT_BLOCK_SHIFT;
        ...
        for (BytesRef term = iterator.next(); term != null; term = iterator.next()) {
            if ((ord & FqlDocValuesFormat.TERMS_DICT_BLOCK_MASK) == 0) {
    ...
    private void writeTermsIndex(SortedSetDocValues values) throws IOException {
        final long size = values.getValueCount();
        meta.writeInt(FqlDocValuesFormat.TERMS_DICT_REVERSE_INDEX_SHIFT);
        long start = data.getFilePointer();

        long numBlocks = 1L + ((size + FqlDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) >>> FqlDocValuesFormat.TERMS_DICT_REVERSE_INDEX_SHIFT);
        RAMOutputStream addressBuffer = new RAMOutputStream();
        DirectMonotonicWriter writer = DirectMonotonicWriter.getInstance(meta, addressBuffer, numBlocks, DIRECT_MONOTONIC_BLOCK_SHIFT);
        ...
        long ord = 0;
        for (BytesRef term = iterator.next(); term != null; term = iterator.next()) {
            if ((ord & FqlDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == 0) {
                writer.add(offset);
            ...
            } else if ((ord & FqlDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == FqlDocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) {
                previous.copyBytes(term);
            }
            ++ord;
        }
    ...
    public void addSortedNumericField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(FqlDocValuesFormat.SORTED_NUMERIC);
    ...
    public void addSortedSetField(FieldInfo field, DocValuesProducer valuesProducer) throws IOException {
        meta.writeInt(field.number);
        meta.writeByte(FqlDocValuesFormat.SORTED_SET);
